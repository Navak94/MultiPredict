...
class StockLSTM(nn.Module):
    def __init__(self, input_size=1, hidden_size=256, output_size=1):  # Reduced hidden size
...
learning_rate = 0.005
clip_value = 1.0
batch_size = 32  # Added batch size for GPU optimization
...
torch.set_float32_matmul_precision('medium')  # Adjusting matrix precision
torch.cuda.set_per_process_memory_fraction(0.5, device=device)  # Limit GPU memory usage to 50%
...
for Company in companies:
    ...
    # Normalize data using CPU and then move to GPU
    ...
    model = StockLSTM(input_size=1, hidden_size=256).to(device)  # Reduced hidden size
    criterion = nn.L1Loss()
    ...
    # Move predictions back to CPU for inverse transform
    future_predictions = scaler.inverse_transform(np.array(future_predictions).reshape(-1, 1))

    print(f"Tomorrow's Predicted Value for {Company}: {future_predictions[0][0]:.2f}")
    print(f"Predicted Value for {Company} 30 days from tomorrow: {future_predictions[-1][0]:.2f}")

    # Plot results
    plt.figure(figsize=(12, 6))
    plt.plot(data.index[-120:], scaler.inverse_transform(df[-120:]), label="Last 120 Days")
